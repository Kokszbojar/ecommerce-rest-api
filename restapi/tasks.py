...
def send_reminder_mails(recipients):
    ...
    message = str('This is a reminder mail.' +
                  'You have less than 24 hours to pay for your order.')
...
def reminder_mails_task():
    ...
    orders = Order.objects.filter(payment_date__gte=time_now,
                                  payment_date__lte=time_now+timedelta(days=1),
                                  cart_confirmed=True)
    recipients = []
    for order in orders:
